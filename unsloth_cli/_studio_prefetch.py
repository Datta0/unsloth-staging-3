...
def _uv_version(env: Optional[dict]) -> Optional[str]:
    try:
...
def run(
    *,
    ...
    if not shutil.which("uv"):
        raise PrefetchSkipped("uv is not available")
    ...
    core_cmd = core_dry_run_command(interpreter, floor = floor, constraints = live_constraints)
    try:
    ...
    payload: dict = {
        ...
        "uv_version": _uv_version(child_env),
    ...
    fetch_cmd = fetch_command(interpreter, target, pins)
    try:
...
def _prefetch_requirement_file(
    ...
    *,
    ...
    try:
        resolved = _run(
            requirement_dry_run_command(
                interpreter, requirement, constraints = constraints, no_deps = no_deps
            ),
            env,
        )
    except (OSError, subprocess.SubprocessError) as exc:
    ...
    try:
        ...
        fetched = _run(
            fetch_command(interpreter, target, pins_from_plan(planned), only_binary = True),
            env,
        )
    except (OSError, subprocess.SubprocessError) as exc:
