...
async def _create_openai_video(
    ...
    # begin_generate hands back the canvas it resolved. Without it a reference-image request reported the family's
    # first preset while the clip rendered at the source aspect, so the job advertised one size and the finished
    # record another.
