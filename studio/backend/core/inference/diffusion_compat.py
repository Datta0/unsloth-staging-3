...
# The FLUX.2 tensor table sits in the first ~15 KiB (149 tensors for klein-4B, 201 for 9B). This is the ceiling on what
# the range request may buffer, not an expectation: a prefix that stops mid-table makes the parse raise, which reads as
# "no opinion".
_GGUF_HEADER_BYTES = 256 * 1024
# One short read. A pick is blocked on this in the UI, and a slow Hub must not stall the picker; a timeout is just
# another fail-open.
...
# memoises the MISS too -- the three checks on one pick would otherwise re-probe an unreachable Hub three times, and a
# sticky None is the degradation this module promises anyway. Which is exactly why the last two key parts exist: a
# sticky None must not outlive its cause.    * the TOKEN, fingerprinted rather than stored. Keying on mere presence made
# every non-empty     token one key, so a first probe with an expired credential poisoned the valid one that
# replaced it for the rest of the process.   * the local file's IDENTITY (path, size, mtime). A checkpoint swapped in
# place keeps its path,     so keying on the name alone answers the new file with the old file's dim -- refusing a valid
# 9B pairing, or handing sd.cpp the 4B text encoders. It also makes the file ARRIVING a new     key, so a miss taken
# before a download finished re-probes off disk for free.
...
# twice its documented bound. They share the read now, keyed and aged exactly like the speech memo beside it, so this
# adds no staleness the module did not already accept. Deliberately NOT consulted by the revalidation paths: their whole
# job is to re-read a file the Hub has republished, and answering those from a memo would defeat them.
...
def _local_gguf_path(repo_id: str, gguf_filename: str) -> Optional[str]:
    ...
    try:
        ...
        # The live root first, then huggingface_hub's import-time constant, the same pair the loader resolves a staged
        # file through. Read directly rather than through ``diffusion.hub_cache_dir``: that module imports this one.
...
def _ranged_stream(session: Any, url: str, headers: dict) -> Any:
    ...
    if callable(getattr(session, "stream", None)):
        # httpx does not follow redirects by default and the Hub answers a resolve URL with a 302 to the CDN, so an
        # unfollowed hop would read as "not 206" and fail open.
...
def _read_gguf_header(repo_id: str, gguf_filename: str, hf_token: Optional[str]) -> bytes:
    ...
    # list rather than a nonlocal, so the worker's assignment is visible.
    ...
    def _fetch() -> None:
        try:
            ...
            with _ranged_stream(
                ...
                holder[0] = response
                # 206 or nothing: a server that ignored the Range header answers 200 with the WHOLE checkpoint
                # 206 or nothing. A server (or a proxy) that ignored the Range header answers 200 with the WHOLE
                # checkpoint, and streaming that into memory is the multi-GB download this preflight exists to prevent.
                if response.status_code != 206:
                    return
                deadline = time.monotonic() + _HEADER_TIMEOUT_SECONDS
                for chunk in _iter_body(response, 65536):
                    # extend, not `+=`: augmented assignment to a closed-over name would rebind it as a local and lose
                    # every byte
                    buffer.extend(chunk)
                    if len(buffer) >= _GGUF_HEADER_BYTES or time.monotonic() > deadline:
                        break
        # keep what arrived: a deadline firing on a merely SLOW link still leaves the tensor table in hand
        # Keep what arrived rather than discarding it: the deadline firing on a merely SLOW link still leaves the tensor
        # table (the first ~15 KiB) in hand, and the parser is truncation-safe -- swept over every prefix length of five
    ...
    # arrived and every dribbled byte resets the socket timeout, so the worker cannot notice its own deadline.
    # Half-closing the socket is what makes that read return -- on urllib3 >= 2.3, where HTTPResponse.shutdown exists.
    # requirements/studio.txt floors it, but an install predating that floor keeps whatever it resolved, so the bound
    ...
    watchdog.cancel()
    # bytes() snapshots under the GIL, so an abandoned worker still appending cannot tear the copy; it can only lose a
    # chunk that arrived too late to matter.
...
def flux2_inner_dim_for_pick(
    ...
    *,
    ...
    # A ".gguf" name only: a single_file load names a .safetensors, which has no GGUF header, and spending a range
    # request to learn that on every such load is pure waste.
    if not repo_id or not gguf_filename or not gguf_filename.lower().endswith(".gguf"):
        return None
    token = (hf_token or "").strip() or None
    # Resolved BEFORE the memo is consulted, because the file's identity is part of the key. Two stats, against a probe
    # that is otherwise an HTTP round trip.
    local = _local_gguf_path(repo_id, gguf_filename)
    key = (repo_id, gguf_filename, _token_fingerprint(token), _file_identity(local))
    # The memo FIRST, before the offline bail below. A plan-time probe has usually already answered for this exact pick,
    # and returning None here anyway made the caller that cannot wait (begin_load, allow_network = False) fall back to
    # the filename heuristic -- publishing the 4B encoder repos for a renamed 9B checkpoint, so the delete-cached guard
    # did not cover its real companion repo until the worker re-probed.
    with _CACHE_LOCK:
    ...
    if local is not None:
        # same prefix parse as the remote path: the loader's backstop memory-maps the whole multi-GB checkpoint for a
        # table in the first 15 KiB
        # Same prefix parse as the remote path, so both read the file the same way: the loader's backstop memory-maps
        # the whole multi-GB checkpoint and builds a view over every tensor, which is a lot of work for a table in the
        # first 15 KiB. Fall back to it only if the prefix said nothing, so a header past the cap is still answered.
    ...
    with _CACHE_LOCK:
        # Plain FIFO-ish eviction: this only bounds a session's worth of picks, and a re-probe after an eviction costs
        # one range request.
...
def flux2_pick_mismatch(
    ...
    # Cheapest order: a base outside the size table (a local path, a repo we do not ship) leaves nothing to compare
    # against, so it must not cost a round trip either.
    if want is None:
        return None
...
# GGUF ``general.architecture`` values nothing in Unsloth can decode. Beside the FLUX.2 check because both ask whether
# the pick is loadable, off the same prefix. The set itself lives in a leaf module, shared with the chat gate and the
# listing classifier so they cannot drift.
...
# every remote-backed verdict ages out.
# Every remote-backed verdict ages out. An UNCACHED one keys on a local identity of None, so a republish under the same
# filename changes nothing about the key. A SNAPSHOT-backed one keys on the file's identity, which a republish does
# change -- but only once the new bytes are down, and the entry memoises a revision check that ran only the first time,
# so holding it forever means never asking the Hub again for the life of the process. Only a true On Device checkpoint
# is permanent: it is the file the loader opens, so there is no revision to be behind. Matches the variant listing's own
# freshness window for moved revisions.
...
def _shared_gguf_header(
    ...
    # An empty prefix is a failed read, and the two probes disagreeing about that is not worth a sticky miss: each still
    # memoises its own "no verdict" on its own terms.
    if not prefix:
        return prefix
    with _CACHE_LOCK:
...
def _arch_from_prefix(prefix: bytes, gguf_filename: str) -> Optional[str]:
    ...
    try:
        ...
        with tempfile.TemporaryDirectory(prefix = "unsloth-speech-probe-") as probe_dir:
            # Named after the real file, like the chat-side probe: a GGUF declaring no architecture is judged by its
            # name, which a temp name would lose.
...
def _revalidated_speech_arch(
    ...
    if not allow_network:
        # A cache-only caller cannot wear the HEAD; the caller declines to memoise this answer, so the next one that CAN
        # reach the Hub still revalidates it.
        return arch
    live = _hub_revision(repo_id, gguf_filename, token)
    if live is None or live == cached:
        return arch
    refreshed = _arch_from_prefix(_read_gguf_header(repo_id, gguf_filename, token), gguf_filename)
    # a re-read that said nothing keeps the verdict we had
    # A re-read that said nothing -- failed range request, or an unparseable new header -- keeps the verdict we had
    # rather than replacing it with silence. Failing open on an UNKNOWN pick is the contract; throwing away a known one
    # let a csm file through on a dropped connection.
...
def _speech_probe_architecture(
    ...
    if local is None and not allow_network:
        # Memo or local header only, as the size pairing does. Nothing is memoised, so the next caller that CAN wait
        # still gets a real answer instead of this one's silence.
        return None
    prefix = (
    ...
    # Inside the memo, so a republished checkpoint is caught in either direction and the HEAD is spent once per cached
    # copy rather than on every pick.
    arch = _revalidated_speech_arch(repo_id, gguf_filename, token, local, arch, allow_network)
    # A cached copy whose revision check was skipped is only HALF an answer, so it must not be memoised: the
    # network-allowed caller behind it would read this back and never revalidate.
    ...
    with _CACHE_LOCK:
        ...
        # Permanent only for a true On Device file, which has no revision to be behind. A cached Hub snapshot ages out
        # like an uncached pick: its entry memoises a revision check, and holding that forever would ask the Hub exactly
...
def speech_pick_refusal(
    ...
    if is_speech_gguf_architecture(arch):
        # Named only when the header carried an identifier: the Mimi vocoder puts a whole sentence in
        # general.architecture, and quoting that back reads as gibberish.
