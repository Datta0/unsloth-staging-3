...
def _isolated_scan(monkeypatch, tmp_path, scan_folders):
    """Point every cache/LM Studio lookup at empty dirs so only *scan_folders*
    (registered custom folders) contribute rows."""
    ...
    monkeypatch.setattr(
        ...
        lambda: [{"path": str(folder)} for folder in scan_folders],
    )


def _cache_snapshot(root: Path, repo_dir_name: str, files: dict) -> Path:
    snapshot = root / repo_dir_name / "snapshots" / "rev"
    snapshot.mkdir(parents = True)
    for name, size in files.items():
        (snapshot / name).write_bytes(b"x" * size)
    return snapshot


def test_inactive_hf_cache_gguf_snapshot_carries_a_format_hint(monkeypatch, tmp_path):
    """A scan folder that is itself an HF cache surfaces its repos as "custom"
    rows addressed by snapshot path. A GGUF repo whose name lacks a -GGUF suffix
    has no other format signal, so without the hint the picker and auto-load
    treat it as a plain checkpoint and let the backend pick the largest .gguf
    instead of the smallest downloaded variant."""
    scan = tmp_path / "scan"
    _cache_snapshot(
        scan,
        "models--Org--TinyModel",
        {"tiny-Q4_K_M.gguf": 10, "tiny-Q8_0.gguf": 40, "config.json": 2},
    )
    _isolated_scan(monkeypatch, tmp_path, [scan])

    rows = models_route.collect_local_models(tmp_path / "models")
    [row] = [r for r in rows if r.model_id == "Org/TinyModel" and r.source == "custom"]

    assert row.partial is False
    assert row.model_format == "gguf"


def test_inactive_hf_cache_checkpoint_keeps_no_format_hint(monkeypatch, tmp_path):
    """A real safetensors checkpoint must not be mislabelled as GGUF."""
    scan = tmp_path / "scan"
    _cache_snapshot(scan, "models--Org--Plain", {"model.safetensors": 40, "config.json": 2})
    _isolated_scan(monkeypatch, tmp_path, [scan])

    rows = models_route.collect_local_models(tmp_path / "models")
    [row] = [r for r in rows if r.model_id == "Org/Plain" and r.source == "custom"]

    assert row.model_format is None


def test_inactive_hf_cache_mixed_snapshot_keeps_no_format_hint(monkeypatch, tmp_path):
    """A snapshot holding both .gguf and .safetensors is not a GGUF-only folder,
    so the hint stays off exactly like _dir_model_format's other callers."""
    scan = tmp_path / "scan"
    _cache_snapshot(scan, "models--Org--Mixed", {"mixed-Q4_K_M.gguf": 10, "model.safetensors": 40})
    _isolated_scan(monkeypatch, tmp_path, [scan])

    rows = models_route.collect_local_models(tmp_path / "models")
    [row] = [r for r in rows if r.model_id == "Org/Mixed" and r.source == "custom"]

    assert row.model_format is None


def test_active_hf_cache_rows_are_unchanged(monkeypatch, tmp_path):
    """The active cache is addressed by repo id and its rows are excluded from
    auto-load's local sweep, so they keep their existing shape."""
    active = tmp_path / "active"
    _cache_snapshot(active, "models--Org--TinyModel", {"tiny-Q4_K_M.gguf": 10})
    monkeypatch.setattr(models_route, "_resolve_hf_cache_dir", lambda: active)
    monkeypatch.setattr("utils.paths.legacy_hf_cache_dir", lambda: tmp_path / "legacy")
    monkeypatch.setattr("utils.paths.hf_default_cache_dir", lambda: tmp_path / "default")
    monkeypatch.setattr("utils.paths.lmstudio_model_dirs", lambda: [])
    ...
    rows = models_route.collect_local_models(tmp_path / "models")
    [row] = [r for r in rows if r.model_id == "Org/TinyModel"]

    assert row.source == "hf_cache"
    assert row.id == "Org/TinyModel"
    assert row.active_cache is True
    assert row.model_format is None
