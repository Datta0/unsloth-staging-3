...
def _run_sh_route(cell: dict, arm: str) -> dict:
    ...
    with tempfile.TemporaryDirectory() as d:
        ...
        # A file, not `bash -c`: Windows mangles a long quoted argument on its way into Git Bash.
        sh = os.path.join(d, "cell.sh")
        with open(sh, "w", encoding="utf-8", newline="\n") as fh:
            fh.write(script)
        r = subprocess.run([BASH, _posix(sh)], capture_output=True, text=True, timeout=120)
