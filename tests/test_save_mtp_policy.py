...
def save_module():
    ...
    # `unsloth` first, then the submodule. Importing `unsloth.save` directly on a
    # cold interpreter walks into unsloth.models.vision, which imports back into
    # a half initialised unsloth.save; the package __init__ orders that
    # correctly. Skip rather than error when the whole stack is unavailable, for
    # instance a runner with no triton wheel, so this file reports a reason
    # instead of one setup error per test.
    pytest.importorskip("unsloth", reason = "unsloth is not importable on this runner")
    module = pytest.importorskip(
        "unsloth.save", reason = "unsloth.save is not importable on this runner",
    )
    return module
